...
parser = argparse.ArgumentParser(description='Pamir Example : WiFi Management MCP Server (Linux)')
...
async def handle_call_tool(
    ...
    NR>1 {
        ...
        if result.returncode != 0:
            logger.error(f"Command failed for get_wifi_networks (Linux). Return code: {result.returncode}, Stderr: {result.stderr}")
            ...
            logger.debug(f"get_wifi_networks stdout (Linux): {result.stdout[:200]}...") # Log truncated output
            return [types.TextContent(
                type="text",
        ...
        logger.debug(f"Running command for get_wifi_status (Linux): {' '.join(command_list)}")
        result = subprocess.run(command_list, capture_output=True, text=True, check=False)
        
        if result.returncode != 0:
             logger.error(f"Command failed for get_wifi_status (Linux). Return code: {result.returncode}, Stderr: {result.stderr}")
             return [types.TextContent(
                type="text",
            ...
            for line in result.stdout.splitlines():
                ...
                if any(key in line for key in ["GENERAL.DEVICE", "GENERAL.TYPE", "GENERAL.STATE", 
        ...
        if not ssid or not password:
             logger.warning("connect_to_wifi (Linux) called with missing ssid or password.")
        ...
        try:
            ...
            logger.debug(f"Running command to enable WiFi (Linux): {' '.join(power_command)}")
            ...
            if power_result.returncode != 0 and "already enabled" not in power_result.stderr.lower():
                logger.error(f"Failed to enable WiFi (Linux). Return code: {power_result.returncode}, Stderr: {power_result.stderr}")
                return [types.TextContent(
                    type="text",
                    text=f"Failed to enable WiFi: {power_result.stderr}"
                )]
            logger.info("WiFi power enabled successfully (Linux)." if power_result.returncode == 0 else "WiFi already enabled (Linux).")

            # Ensure password is a string
            password_str = str(password)
            
            # Connect to network (Linux nmcli)
            connect_command = ["sudo", "nmcli", "device", "wifi", "connect", str(ssid), "password", password_str]
            # Mask password in log
            masked_command_str = f"sudo nmcli device wifi connect '{ssid}' password ****"
            logger.debug(f"Running command to connect to WiFi (Linux): {masked_command_str}")
            ...
            if connect_result.returncode == 0:
                logger.info(f"Successfully initiated connection to WiFi network '{ssid}' (Linux).")
                return [types.TextContent(
                    type="text",
                ...
                logger.error(f"Failed to connect to WiFi network '{ssid}' (Linux). Return code: {connect_result.returncode}, Stderr: {error_message}")
                # Provide a cleaner error message
                if "invalid password" in error_message.lower() or "secrets were required" in error_message.lower():
                    user_error = "Failed to connect: Incorrect password?"
                elif "network not found" in error_message.lower() or "No network with SSID" in error_message:
            ...
            logger.error(f"Unexpected error connecting to WiFi '{ssid}' (Linux): {e}", exc_info=True)
            return [types.TextContent(
                type="text",
